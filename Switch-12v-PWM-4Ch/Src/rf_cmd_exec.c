...
void rfCmdExec(uint8_t *Data, uint8_t Length) {
  ...
  if (tDelay > 0) {
    ...
    if (Brightness_c1 == 0x00) {  // Turn off the light
      htim3.Instance->CCR4 = 0x00;
    }
    if (Brightness_c1 > 0x00) {  // Update PWM according to value
    ...
    if (Brightness_c2 == 0x00) {  // Turn off the light
      htim14.Instance->CCR1 = 0x00;
    }
    if (Brightness_c2 > 0x00) {  // Update PWM according to value
